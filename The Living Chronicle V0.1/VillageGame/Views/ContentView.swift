...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .ignoresSafeArea()
        .onAppear {
            // Check if this is a fresh install (no save exists)
...
struct HUDOverlay: View {
    ...
    private var daysLeftInSeason: Int {
        let daysPerSeason = 42
        return max(0, daysPerSeason - viewModel.village.dayInSeason)
    }
    
    var body: some View {
        VStack(spacing: 0) {
            // Top Bar - add safe area padding
            topBar
    ...
    private var topBar: some View {
        HStack(spacing: 12) {
            // Village Name & Era - SINGLE LOCATION
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text("\(daysLeftInSeason) days left")
                        .font(.system(size: 9))
                        .opacity(0.6)
                }
            }
            ...
            // Guild Marks (Premium Currency) - Make it tappable to open shop
...
struct ResourceBar: View {
    let icon: String
    let name: String  // Added name parameter
    let color: Color
    let value: Double
    let label: String?
