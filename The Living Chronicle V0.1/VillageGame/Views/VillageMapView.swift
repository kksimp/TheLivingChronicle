...
struct ProceduralBuildingView: View {
    ...
    private func drawCampfire(context: GraphicsContext, size: CGSize) {
        // Fire pit circle
        let pitRect = CGRect(x: size.width * 0.2, y: size.height * 0.5, width: size.width * 0.6, height: size.height * 0.4)
        context.fill(Path(ellipseIn: pitRect), with: .color(Color(red: 0.3, green: 0.25, blue: 0.2)))
        
        // Stones around fire
        let stoneCount = 6
        for i in 0..<stoneCount {
            let angle = (Double(i) / Double(stoneCount)) * .pi * 2
            let stoneX = size.width * 0.5 + cos(angle) * size.width * 0.25 - 4
            let stoneY = size.height * 0.65 + sin(angle) * size.height * 0.15 - 3
            let stone = Path(ellipseIn: CGRect(x: stoneX, y: stoneY, width: 8, height: 6))
            context.fill(stone, with: .color(Color(red: 0.5, green: 0.5, blue: 0.5)))
        }
        
        // Fire flames - outer
        var flame = Path()
        flame.move(to: CGPoint(x: size.width * 0.5, y: size.height * 0.7))
        flame.addQuadCurve(
            to: CGPoint(x: size.width * 0.35, y: size.height * 0.25),
            control: CGPoint(x: size.width * 0.25, y: size.height * 0.5)
        )
        flame.addQuadCurve(
            to: CGPoint(x: size.width * 0.5, y: size.height * 0.15),
            control: CGPoint(x: size.width * 0.42, y: size.height * 0.18)
        )
        flame.addQuadCurve(
            to: CGPoint(x: size.width * 0.65, y: size.height * 0.25),
            control: CGPoint(x: size.width * 0.58, y: size.height * 0.18)
        )
        flame.addQuadCurve(
            to: CGPoint(x: size.width * 0.5, y: size.height * 0.7),
            control: CGPoint(x: size.width * 0.75, y: size.height * 0.5)
        )
        context.fill(flame, with: .color(Color.orange))
        
        // Inner flame - yellow
        var innerFlame = Path()
        innerFlame.move(to: CGPoint(x: size.width * 0.5, y: size.height * 0.65))
        innerFlame.addQuadCurve(
            to: CGPoint(x: size.width * 0.5, y: size.height * 0.3),
            control: CGPoint(x: size.width * 0.38, y: size.height * 0.45)
        )
        innerFlame.addQuadCurve(
            to: CGPoint(x: size.width * 0.5, y: size.height * 0.65),
            control: CGPoint(x: size.width * 0.62, y: size.height * 0.45)
        )
        context.fill(innerFlame, with: .color(Color.yellow))
    }

    private func drawGatheringPost(context: GraphicsContext, size: CGSize) {
        // Main post
        let postRect = CGRect(x: size.width * 0.45, y: size.height * 0.15, width: size.width * 0.1, height: size.height * 0.75)
        context.fill(Path(roundedRect: postRect, cornerRadius: 2), with: .color(Color(red: 0.5, green: 0.35, blue: 0.2)))
        
        // Crossbeam
        let beamRect = CGRect(x: size.width * 0.15, y: size.height * 0.2, width: size.width * 0.7, height: size.height * 0.08)
        context.fill(Path(roundedRect: beamRect, cornerRadius: 2), with: .color(Color(red: 0.45, green: 0.3, blue: 0.15)))
        
        // Left basket
        var basket1 = Path()
        basket1.move(to: CGPoint(x: size.width * 0.15, y: size.height * 0.3))
        basket1.addLine(to: CGPoint(x: size.width * 0.1, y: size.height * 0.6))
        basket1.addQuadCurve(
            to: CGPoint(x: size.width * 0.35, y: size.height * 0.6),
            control: CGPoint(x: size.width * 0.22, y: size.height * 0.68)
        )
        basket1.addLine(to: CGPoint(x: size.width * 0.3, y: size.height * 0.3))
        context.fill(basket1, with: .color(Color(red: 0.6, green: 0.5, blue: 0.3)))
        
        // Right basket
        var basket2 = Path()
        basket2.move(to: CGPoint(x: size.width * 0.7, y: size.height * 0.3))
        basket2.addLine(to: CGPoint(x: size.width * 0.65, y: size.height * 0.6))
        basket2.addQuadCurve(
            to: CGPoint(x: size.width * 0.9, y: size.height * 0.6),
            control: CGPoint(x: size.width * 0.78, y: size.height * 0.68)
        )
        basket2.addLine(to: CGPoint(x: size.width * 0.85, y: size.height * 0.3))
        context.fill(basket2, with: .color(Color(red: 0.6, green: 0.5, blue: 0.3)))
        
        // Berries/items in baskets
        let berryColors: [Color] = [.red, .purple, Color(red: 0.3, green: 0.5, blue: 0.2)]
        for i in 0..<4 {
            let x1 = size.width * 0.15 + CGFloat(i % 2) * 10
            let y1 = size.height * 0.4 + CGFloat(i / 2) * 8
            let berry1 = Path(ellipseIn: CGRect(x: x1, y: y1, width: 6, height: 6))
            context.fill(berry1, with: .color(berryColors[i % berryColors.count]))
            
            let x2 = size.width * 0.7 + CGFloat(i % 2) * 10
            let berry2 = Path(ellipseIn: CGRect(x: x2, y: y1, width: 6, height: 6))
            context.fill(berry2, with: .color(berryColors[(i + 1) % berryColors.count]))
        }
    }

    private func drawFishingSpot(context: GraphicsContext, size: CGSize) {
        // Water area
        let waterRect = CGRect(x: 0, y: size.height * 0.45, width: size.width, height: size.height * 0.55)
        context.fill(Path(roundedRect: waterRect, cornerRadius: 10), with: .color(Color(red: 0.25, green: 0.45, blue: 0.65)))
        
        // Water ripples
        for i in 0..<3 {
            let rippleY = size.height * 0.55 + CGFloat(i) * 12
            var ripple = Path()
            ripple.move(to: CGPoint(x: size.width * 0.2, y: rippleY))
            ripple.addQuadCurve(
                to: CGPoint(x: size.width * 0.8, y: rippleY),
                control: CGPoint(x: size.width * 0.5, y: rippleY - 5)
            )
            context.stroke(ripple, with: .color(Color.white.opacity(0.3)), lineWidth: 1)
        }
        
        // Wooden dock
        let dockRect = CGRect(x: size.width * 0.35, y: size.height * 0.25, width: size.width * 0.15, height: size.height * 0.55)
        context.fill(Path(roundedRect: dockRect, cornerRadius: 2), with: .color(Color(red: 0.55, green: 0.4, blue: 0.25)))
        
        // Dock planks
        for i in 0..<4 {
            let plankY = size.height * 0.3 + CGFloat(i) * 12
            let plankRect = CGRect(x: size.width * 0.33, y: plankY, width: size.width * 0.19, height: 3)
            context.fill(Path(roundedRect: plankRect, cornerRadius: 1), with: .color(Color(red: 0.45, green: 0.32, blue: 0.18)))
        }
        
        // Fishing pole
        var pole = Path()
        pole.move(to: CGPoint(x: size.width * 0.42, y: size.height * 0.35))
        pole.addLine(to: CGPoint(x: size.width * 0.75, y: size.height * 0.05))
        context.stroke(pole, with: .color(Color(red: 0.45, green: 0.35, blue: 0.2)), lineWidth: 3)
        
        // Fishing line
        var line = Path()
        line.move(to: CGPoint(x: size.width * 0.75, y: size.height * 0.05))
        line.addQuadCurve(
            to: CGPoint(x: size.width * 0.7, y: size.height * 0.55),
            control: CGPoint(x: size.width * 0.85, y: size.height * 0.3)
        )
        context.stroke(line, with: .color(Color.white.opacity(0.5)), lineWidth: 1)
        
        // Bobber
        let bobber = Path(ellipseIn: CGRect(x: size.width * 0.67, y: size.height * 0.52, width: 8, height: 8))
        context.fill(bobber, with: .color(Color.red))
        
        // Fish shadows underwater
        for i in 0..<2 {
            var fish = Path()
            let fishX = size.width * (0.15 + CGFloat(i) * 0.4)
            let fishY = size.height * 0.7
            fish.move(to: CGPoint(x: fishX, y: fishY))
            fish.addQuadCurve(
                to: CGPoint(x: fishX + 12, y: fishY),
                control: CGPoint(x: fishX + 6, y: fishY - 4)
            )
            fish.addLine(to: CGPoint(x: fishX + 16, y: fishY - 3))
            fish.addLine(to: CGPoint(x: fishX + 16, y: fishY + 3))
            fish.addLine(to: CGPoint(x: fishX + 12, y: fishY))
            fish.addQuadCurve(
                to: CGPoint(x: fishX, y: fishY),
                control: CGPoint(x: fishX + 6, y: fishY + 4)
            )
            context.fill(fish, with: .color(Color.white.opacity(0.25)))
        }
    }
    ...
    private func drawBuilding(context: GraphicsContext, size: CGSize) {
        ...
        case .campfire:
            drawCampfire(context: context, size: size)
        case .gatheringPost:
            drawGatheringPost(context: context, size: size)
        case .fishingSpot:
            drawFishingSpot(context: context, size: size)
        case .farm:
    ...
    private func drawShelter(context: GraphicsContext, size: CGSize) {
        // Sleeping bag shape - elongated rounded rectangle
        let bagWidth = size.width * 0.8
        let bagHeight = size.height * 0.4
        let bagY = size.height * 0.5
        
        // Main sleeping bag body
        let bagRect = CGRect(
            x: (size.width - bagWidth) / 2,
            y: bagY,
            width: bagWidth,
            height: bagHeight
        )
        
        // Bag body with gradient effect
        let bagColor = Color(red: 0.3, green: 0.4, blue: 0.2) // Olive/forest green
        context.fill(
            Path(roundedRect: bagRect, cornerRadius: bagHeight / 2),
            with: .color(bagColor)
        )
        ...
        context.fill(
            Path(roundedRect: bottomRect, cornerRadius: bagHeight / 4),
            with: .color(Color(red: 0.25, green: 0.35, blue: 0.15))
        )
        
        // Hood/pillow area at top
        let hoodRect = CGRect(
            x: bagRect.minX - 5,
            y: bagY - bagHeight * 0.3,
            width: bagWidth * 0.4,
            height: bagHeight * 0.5
        )
        context.fill(
            Path(roundedRect: hoodRect, cornerRadius: 8),
            with: .color(Color(red: 0.35, green: 0.45, blue: 0.25))
        )
        
        // Zipper line
        var zipper = Path()
        zipper.move(to: CGPoint(x: size.width / 2, y: bagY + 5))
        zipper.addLine(to: CGPoint(x: size.width / 2, y: bagY + bagHeight - 5))
        context.stroke(zipper, with: .color(Color.gray.opacity(0.6)), lineWidth: 2)
        
        // Small person shape peeking out (just a head circle)
        ...
        context.fill(headCircle, with: .color(Color(red: 0.85, green: 0.7, blue: 0.55)))
    }
...
struct AnimatedVillagerView: View {
    ...
    private let skinTones: [Color] = [
        Color(red: 0.96, green: 0.87, blue: 0.78), // Light
        Color(red: 0.87, green: 0.75, blue: 0.65), // Fair
        Color(red: 0.78, green: 0.63, blue: 0.52), // Medium light
        Color(red: 0.68, green: 0.52, blue: 0.40), // Medium
        Color(red: 0.55, green: 0.40, blue: 0.30), // Medium dark
        Color(red: 0.45, green: 0.32, blue: 0.22), // Dark
        Color(red: 0.35, green: 0.24, blue: 0.16), // Deep
        Color(red: 0.82, green: 0.68, blue: 0.55), // Olive
    ]
    
    private var skinTone: Color {
        let hash = seededRandom(seed: seed, index: index * 7)
        return skinTones[Int(hash % UInt64(skinTones.count))]
    }
    
    private var hairColor: Color {
        let hairColors: [Color] = [
            Color(red: 0.1, green: 0.1, blue: 0.1),   // Black
            Color(red: 0.3, green: 0.2, blue: 0.15),  // Dark brown
            Color(red: 0.45, green: 0.3, blue: 0.2),  // Brown
            Color(red: 0.6, green: 0.4, blue: 0.2),   // Light brown
            Color(red: 0.8, green: 0.6, blue: 0.3),   // Blonde
            Color(red: 0.6, green: 0.3, blue: 0.1),   // Auburn
            Color(red: 0.5, green: 0.5, blue: 0.5),   // Gray
        ]
        let hash = seededRandom(seed: seed, index: index * 13)
        return hairColors[Int(hash % UInt64(hairColors.count))]
    }
